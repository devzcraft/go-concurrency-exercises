...
const expireAfter = time.Second * 5
...
func (m *SessionManager) DeleteSessionData(sessionID string) error {
	_, ok := m.sessions[sessionID]
	if !ok {
		return ErrSessionNotFound
	}
...
func (m *SessionManager) Clean() {
	...
	for id, s := range m.sessions {
		if time.Since(s.Updated) > expireAfter {
			err := m.DeleteSessionData(id)
			if err != nil {
				panic(err)
			}
		}
